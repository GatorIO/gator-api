...
exports.attributes = [];
function init(_defaultAppId, callback) {
    try {
        exports.defaultAppId = _defaultAppId;
        applications.getAll(function (err, apps) {
            if (err) {
                console.log('Error in reporting.init: ' + err.message);
                logger.log('AppId: ' + exports.defaultAppId, err);
                callback(err);
                return;
            }
            async.each(apps, function (app, asyncCallback) {
                if (!app.reporting) {
                    asyncCallback();
                }
                else {
                    ...
                    REST.client.get(app.reporting.apiEndpoint + 'attributes', function (err, apiRequest, apiResponse, result) {
                        if (err) {
                            logger.log('AppId: ' + exports.defaultAppId, err);
                            ...
                            exports.attributes[app.id] = result.data;
                            console.log('Reporting.init successful for ' + app.name + ' - attribute count: ' + exports.attributes[app.id].length);
                            asyncCallback();
                        }
                    });
                }
...
function applicationAttributes(appId) {
    if (typeof appId != 'undefined')
        return exports.attributes[appId];
    return exports.attributes[exports.defaultAppId];
}
exports.applicationAttributes = applicationAttributes;
function getAttributes(view, attributeType, isLog, appId) {
...
function getFilterOptions(view, customAttribs, isLog, appId) {
...
function getSegments(req, useCache, appId, callback) {
    try {
        ...
        var useAppId_1 = exports.defaultAppId;
        if (typeof appId != "undefined")
            useAppId_1 = +appId;
        applications.getAll(function (err, apps) {
            var endpoint = apps[useAppId_1].reporting.apiEndpoint;
            REST.client.get(endpoint + 'segments?accessToken=' + req.session['accessToken'], function (err, apiRequest, apiResponse, result) {
                if (!err) {
                    ...
                    logger.log('AppId: ' + useAppId_1, err);
