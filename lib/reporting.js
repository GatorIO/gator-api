...
function initialize(callback) {
    try {
        REST.client.get('/v1/analytics/attributes', function (err, apiRequest, apiResponse, result) {
            if (err)
                callback(err);
            else {
...
function getSegments(req, useCache, callback) {
    try {
        ...
        REST.client.get('/v1/analytics/segments/account/' + req['session'].account.id + '?accessToken=' + req.session['accessToken'], function (err, apiRequest, apiResponse, result) {
            if (!err)
