...
function getAttributes(entityName, attributeType, isLog) {
    var attribs = [], appAttribs = exports.entities[entityName].attributes;
...
function addAttributeView(options, entityName, attributeType, customAttribs) {
    for (var name_1 in customAttribs[entityName]) {
        if (customAttribs[entityName].hasOwnProperty(name_1)) {
            var attrib = customAttribs[entityName][name_1];
            if ((attributeType == AttributeTypes.all) || (attributeType == AttributeTypes.elements && attrib.isElement) || (attributeType == AttributeTypes.metrics && attrib.isMetric)) {
                options.push({
                    value: entityName + '.' + name_1,
                    text: entityName + ': ' + name_1,
                    optgroup: "Custom"
                });
            }
        }
...
function getAttributeOptions(entityName, attributeType, customAttribs, isLog) {
    var options = [], attribs = getAttributes(entityName, attributeType, isLog);
    if (customAttribs) {
...
function addFilterView(filterOptions, entityName, customAttribs) {
    for (var name_2 in customAttribs[entityName]) {
        if (customAttribs[entityName].hasOwnProperty(name_2) && customAttribs[entityName][name_2].filterable) {
            var attrib = customAttribs[entityName][name_2];
            attrib.title = entityName + ': ' + name_2;
            attrib.name = entityName + '.' + name_2;
...
function getFilterOptions(entityName, customAttribs, isLog) {
    ...
    var appAttribs = exports.entities[entityName].attributes;
    for (var a = 0; a < appAttribs.length; a++) {
        attrib = appAttribs[a];
...
function getRuleOptions(entityName) {
    var attrib, ruleOptions = [];
    var appAttribs = exports.entities[entityName].attributes;
    for (var a = 0; a < appAttribs.length; a++) {
        attrib = appAttribs[a];
        if (attrib.rule) {
